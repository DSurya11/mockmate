...
2. **Set your Groq API key and start:**
   ```bash
   # Windows PowerShell
...
**`ai-service/.env` required values:**
```env
...
> **Note:** Piper TTS voices must be present in `/voices/` (`.onnx` files for `en_US-ryan-high`, `en_US-joe-medium`, `en_US-amy-low`). The `download_voices.js` script at the repo root fetches them.
...
├── voices/            Piper TTS .onnx voice model files
